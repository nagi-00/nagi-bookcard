...
`,document.getElementById(`preview`).innerHTML=`<div id="preview-inner"></div>`;var Y=document.getElementById(`preview-inner`),X=new Date;document.getElementById(`date-input`).value=`${X.getFullYear()}-${String(X.getMonth()+1).padStart(2,`0`)}-${String(X.getDate()).padStart(2,`0`)}`,n({date:X});function Z(){ue(le(e.accentColor,e.theme)),ae(Y),Q(),ct(),st()}function st(){let t=document.getElementById(`preview`);e.bgPreset===`image`&&e.bgImage?(t.style.backgroundImage=`url(${e.bgImage})`,t.style.backgroundSize=`cover`,t.style.backgroundPosition=`center`,t.style.backgroundColor=``):(t.style.backgroundImage=``,t.style.backgroundColor=e.bgColor)}function ct(){document.querySelectorAll(`#star-input .star-btn`).forEach((t,n)=>{t.classList.toggle(`on`,n<e.rating)})}function Q(){let e=Y.querySelector(`.card-scene`);if(!e)return;let t=e.offsetWidth,n=e.offsetHeight,r=document.getElementById(`preview`).offsetWidth-80,i=document.getElementById(`preview`).offsetHeight-80,a=Math.min(r/t,i/n,1);Y.style.transform=`scale(${a})`}window.addEventListener(`resize`,Q);var $;document.getElementById(`search-input`).addEventListener(`input`,e=>{clearTimeout($),$=setTimeout(()=>lt(e.target.value),400)}),document.getElementById(`search-btn`).addEventListener(`click`,()=>{lt(document.getElementById(`search-input`).value)});async function lt(e){if(!e.trim())return;let t=document.getElementById(`search-results`);t.innerHTML=`<div style="padding:8px 12px;font-size:12px;color:#8C8680">검색 중...</div>`,t.classList.add(`open`);let r=await se(e,q());if(!r.length){t.innerHTML=`<div style="padding:8px 12px;font-size:12px;color:#8C8680">결과 없음</div>`;return}t.innerHTML=r.map((e,t)=>`
